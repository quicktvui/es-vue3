...
import { HistoryLocation, HistoryState, NavigationType, RouterHistory } from "./history/common";
import {
...
import { createESNativeRouter } from "./es/ESNativeRouter";
...
export function createRouter(options: RouterOptions): Router {
  ...
  //原生路由
  const nativeRouter = createESNativeRouter(isSupportSlotType());
  ...
  function handleRedirectRecord(to: RouteLocation): RouteLocationRaw | void {
    ...
    if (lastMatched && lastMatched.redirect) {
      ...
      if (__DEV__ && !("path" in newTargetLocation) && !("name" in newTargetLocation)) {
        warn(
  ...
  function finalizeNavigation(
    ...
    if (isPush) {
      ...
      if (!isSupportSlotType()) {
        ...
        //clear task
        if (routeLocationMatched.launchMode === ESRouteLaunchMode.ES_ROUTE_LAUNCH_MODE_CLEAR_TASK) {
        ...
        //single task
          ...
          routeLocationMatched.launchMode === ESRouteLaunchMode.ES_ROUTE_LAUNCH_MODE_SINGLE_TASK
        ) {
      ...
      if (
        replace &&
        routeLocationMatched.launchMode !== ESRouteLaunchMode.ES_ROUTE_LAUNCH_MODE_CLEAR_TASK
      ) {
        info("router::replace start...", toLocation);
        routerHistory.replace(
          resolveHistoryLocation(toLocation.key, toLocation.fullPath, toLocation.resultCallback),
          assign(
            {
  ...
  function resolveClearTaskLifecycle(routeLocationMatched) {
    ...
    const historyLocationList: HistoryLocation[] = routerHistory.clearHistoryLocations();
    if (historyLocationList && historyLocationList.length > 0) {
  ...
  //------------------------------------单例--------------------------------------
  function resolveSingleTaskLifecycle(routeLocationMatched, toLocation, replace: boolean) {
    const historyLocationList = routerHistory.getHistoryLocations();
    const length = historyLocationList.length;
    const index = findHistoryLocationIndex(toLocation);
    if (index > -1) {
      const locationList = routerHistory.popHistoryLocations(length - index - 1);
  ...
  function resolveStandardLifecycle(routeLocationMatched, replace?: boolean) {
    ...
    if (historyLocationList && historyLocationList.length > 0) {
      ...
      try {
        ...
        console.log("invoke stack bottom component instance pause stop error...", e);
      }

      const isDialogType = routeLocationMatched.type == ESRouteType.ES_ROUTE_TYPE_DIALOG;
      console.log("RouteLocationMatched: isDialogType", routeLocationMatched, isDialogType);
      ...
      try {
        ...
        console.log("invoke stack top component instance pause stop error...", e);
      }
    }
  }
  ...
  function pauseStopDestroyRoute(
    ...
    if (historyLocation.lifecycle < LifecycleState.pause) {
      ...
      try {
        ...
        console.log("invoke " + historyLocation + " component instance onESPause error...", e);
      }
    }
    //onESStop
    if (stop && historyLocation.lifecycle < LifecycleState.stop) {
      ...
      try {
        ...
        console.log("invoke " + historyLocation + " component instance onESStop error...", e);
      }
    }

    //onESSaveInstanceState
    if (stop && saveInstanceState && historyLocation.lifecycle < LifecycleState.saveInstanceState) {
      historyLocation.lifecycle = LifecycleState.saveInstanceState;
      try {
        if (instance) {
          try {
            triggerESLifecycle(instance, "saveInstanceState", historyLocation.saveInstanceState);
          } catch (e) {}
        ...
        console.log(
          "invoke " + historyLocation + " component instance onESSaveInstanceState error...",
          e,
        );
      }
    }
    ...
    if (stop && destroy && historyLocation.lifecycle < LifecycleState.destroy) {
      ...
      try {
        ...
        console.log("invoke " + historyLocation + " component instance onESDestroy error...", e);
      }
      //
  ...
  function resolveRouteLocationMatched(targetLocation: RouteLocation): RouteLocationMatched {
    let initialDepth = 0;
