...
class DeviceManager {
  ...
  public onAppDisconnect() {
    ...
    info('device-manager: onAppDisconnect:', device)
  ...
  public async onAppConnect() {
    info('device-manager: onAppConnect', this.deviceList)
    ...
    if (device.physicalstatus === DeviceStatus.Connected && useTunnel) {
      try {
        info('device-manager: createTargetByDeviceInfo', device)
        let debugTarget = createTargetByDeviceInfo(device);
        info('device-manager: debugTarget', debugTarget)
  ...
  public async getDeviceList() {
    global.addon.getDeviceList((devices: DeviceInfo[]) => {
      ...
      info('device-manager: getDeviceList:', devices)
      ...
      if (devices.length) {
        ...
        info('device-manager: `selectDevice:', deviceId)
