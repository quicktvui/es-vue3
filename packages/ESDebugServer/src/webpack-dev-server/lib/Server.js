...
const {info, debug} = require("@debug-server-next/logger/log");
...
class Server {
  ...
  addAdditionalEntries(compiler) {
    ...
    const {
      appendEntries: hmrAppendEntries,
      prependEntries: hmrPrependEntries
    } = this.addHMREntries(compiler);
    const {
      appendEntries: vueAppendEntries,
      prependEntries: vuePrependEntries
    } = this.addVueDevtoolsEntries();
    const {
      appendEntries: reactAppendEntries,
      prependEntries: reactPrependEntries
    } = this.addReactDevtoolsEntries();
    const {
      appendEntries: jsAppendEntries,
      prependEntries: jsPrependEntries
  ...
  addHMREntries(compiler) {
    info("webpack-dev-server initialize addHMREntries start...")
    const appendEntries = [];
    ...
    info("webpack-dev-server initialize addHMREntries end...", appendEntries)
  ...
  getCompilerOptions() {
    if (typeof this.compiler.compilers !== 'undefined') {
      info("webpack-dev-server compilers length:" + this.compiler.compilers.length)
      ...
      if (compilerWithDevServer) {
        info("webpack-dev-server compilerWithDevServer:" + compilerWithDevServer.options)
      ...
      if (compilerWithWebPreset) {
        info("webpack-dev-server compilerWithWebPreset:" + compilerWithWebPreset.options)
        return compilerWithWebPreset.options;
      }
      info("webpack-dev-server compilers[0]:" + this.compiler.compilers[0].options)
      // Fallback
      return this.compiler.compilers[0].options;
    }
    info("webpack-dev-server getCompilerOptions end...:" + this.compiler.options)
  ...
  async normalizeOptions() {
    ...
    info("webpack-dev-server method:normalizeOptions() start...")
    ...
    info("webpack-dev-server compilerOptions:", compilerOptions)
    ...
    const getStaticItem = (optionsForStatic) => {
      ...
      const getDefaultStaticOptions = () => ({
        ...
        staticOptions: {},
        publicPath: ['/'],//http://localhost:38989/dist/dev
      ...
      info("webpack-dev-server getStaticItem :", item)
      //日志
      //   [ES Debug Server]: webpack-dev-server options.static: [
      //   {
      //     directory: '/Users/liulipeng/workspace/vue/template/es-vue3/public',
      //     staticOptions: {},
      //     publicPath: [ '/' ],
      //     serveIndex: { icons: true },
      //     watch: {
    ...
    if (typeof options.allowedHosts === 'undefined') {
      ...
      info("webpack-dev-server options.allowedHosts: auto")
    } else if (
    ...
    if (typeof options.bonjour === 'undefined') {
      ...
      info("webpack-dev-server options.bonjour: false")
    ...
    if (typeof options.client === 'undefined' || (typeof options.client === 'object' && options.client !== null)) {
      ...
      info("webpack-dev-server options.client:", options.client)
      ...
      info("webpack-dev-server options.client.overlay:", options.client.overlay)
      ...
      info("webpack-dev-server options.client.reconnect:", options.client.reconnect)
      ...
      info("webpack-dev-server options.client.logging:", options.client.logging)
    }

    if (typeof options.compress === 'undefined') {
      options.compress = true;
    }
    info("webpack-dev-server options.compress:", options.compress)

    if (typeof options.devMiddleware === 'undefined') {
      options.devMiddleware = {};
    }
    info("webpack-dev-server options.devMiddleware:", options.devMiddleware)
    ...
    info("webpack-dev-server options.historyApiFallback:", options.historyApiFallback)

    // No need to normalize `host`
    options.hot = typeof options.hot === 'boolean' || options.hot === 'only' ? options.hot : true;
    info("webpack-dev-server options.hot:", options.hot)
    ...
    info("webpack-dev-server options.server:", options.server)
    ...
    info("webpack-dev-server options.liveReload:", options.liveReload)
    ...
    info("webpack-dev-server options.open:", options.open)

    if (typeof options.port === 'string' && options.port !== 'auto') {
      options.port = Number(options.port);
    }
    info("webpack-dev-server options.port:", options.port)
    ...
    info("webpack-dev-server options.static:", options.static)
    ...
    info("webpack-dev-server options.watchFiles:", options.watchFiles)

    options.host = await Server.getHostname(options.host);
    options.port = await Server.getFreePort(options.port);

    info("webpack-dev-server options.host:", options.host)
    info("webpack-dev-server options.port:", options.port)

    info("webpack-dev-server method:normalizeOptions() end...")
  }

  //----------------------------Progress  插件----------------------------------------
  setupProgressPlugin() {
    info("webpack-dev-server initialize setupProgressPlugin")
    ...
    }).apply(this.compiler);//给 webpack 安装插件
  }

  //------------------------------初始化--------------------------------------
  async initialize() {

    info("webpack-dev-server initialize start...")
    ...
    compilers.forEach((compiler) => {
      ...
      if (this.options.hot) {
        ...
        info("webpack-dev-server initialize HMRPluginExists:" + HMRPluginExists)
        if (HMRPluginExists) {
          this.logger.warn(
          ...
          info("webpack-dev-server initialize apply HotModuleReplacementPlugin")
  ...
  //------------------------------初始化--------------------------------------
  setupApp() {
    info("webpack-dev-server initialize setupApp")
    // Init express server
  ...
  async setupAdbReverse() {
    info("webpack-dev-server initialize setupAdbReverse")
    await saveDevPort(this.options.port);
    await startAdbProxy();
  }

  setupHooks() {
    info("webpack-dev-server initialize setupHooks")
    ...
    //TODO LIULIPENG 编译完成，启动websocket
    this.compiler.hooks.done.tap('webpack-dev-server', async (stats) => {
      info("webpack-dev-server compiler hooks done...")
      if (!this.webSocketClient) {
        //TODO
      ...
      this.stats = stats;
      this.cb(null, stats);
      info("webpack-dev-server sendStatsWithOption start...")
      this.sendStatsWithOption(this.getWebpackStats(stats));
      //
    });
    //TODO LIULIPENG 修改文件后，编译执行
    this.setupEmitHooks();
  }
  ...
  setupEmitHooks() {
    ...
    compiler.hooks.emit.tap('webpack-dev-server', (compilation) => {
      ...
      compiler.hooks.assetEmitted.tap('webpack-dev-server', (file, info) => {
        debug("webpack-dev-server compiler hooks assetEmitted...")
        let content = null;
        let targetName = null;

        if (info.compilation) {
          ({targetPath: targetName, content} = info);
          debug("webpack-dev-server compiler hooks assetEmitted compilation ")
        } else {
          let targetFile = file;
          ...
          content = info;
          debug("webpack-dev-server compiler hooks assetEmitted targetName:" + targetName + "  content:", content)
        }
        ...
        debug("webpack-dev-server compiler hooks assetEmitted emitMap:", this.emitMap)
      });
    });
  }

  setupHostHeaderCheck() {
    info("webpack-dev-server initialize setupHostHeaderCheck")
  ...
  setupDevMiddleware() {
    info("webpack-dev-server initialize setupDevMiddleware")
  ...
  //-----------------------------------本地服务----------------------------------------
  setupBuiltInRoutes() {
    info("webpack-dev-server initialize setupBuiltInRoutes")
  ...
  setupWatchFiles() {
    info("webpack-dev-server initialize setupWatchFiles")
  ...
  setupFeatures() {
    info("webpack-dev-server initialize setupFeatures")
    const features = {
      compress: () => {
  ...
  createServer() {
    info("webpack-dev-server initialize createServer start...")
    ...
    //     }).listen(8080);
    //
    //https://blog.csdn.net/kiddii/article/details/71422115
    this.server = require(this.options.server.type).createServer(this.options.server.options, this.app);
    //--------------------------------------------------------------------------------

    //
    this.server.on('connection', (socket) => {
      info("webpack-dev-server server connection...")
      // Add socket to list
      this.sockets.push(socket);

      socket.once('close', () => {
        info("webpack-dev-server server close...")
    ...
    this.server.on('error', (error) => {
      info("webpack-dev-server server error...", error)
      this.logger.error('start hippy dev server error: ', error);
      throw error;
    });
    info("webpack-dev-server initialize createServer end...")
  }
  ...
  createWebSocketClient() {
    info("webpack-dev-server WebSocket start...")
    if (!this.options.remote) return;
    return new Promise((resolve, reject) => {
      ...
      //ws://127.0.0.1:38989/debugger-proxy?role=hmr_server&hash=8a41d9dff0d5575f06c92d06e1f586ac
      info("webpack-dev-server WebSocket webSocketURL:", webSocketURL)

      this.webSocketClient = new WebSocket(webSocketURL);
      this.webSocketClient.on('open', () => {
        info("webpack-dev-server WebSocket open...")
...
//-----------------------------------------------------------------------------------------
  async sendStatsWithOption() {
    ...
    info("webpack-dev-server WebSocket sendStatsWithOption start...", stats)
    const messages = [
      {
        type: HMREvent.Hash,
        data: stats.hash,
    ...
    info("webpack-dev-server WebSocket sendStatsWithOption:", allResource)
  ...
  async sendMessage(hmrWsData) {
    info("webpack-dev-server WebSocket sendMessage:", hmrWsData)
    //
    if (this.webSocketClient) {
  ...
  getEmitList() {
    ...
    info("webpack-dev-server WebSocket getEmitList:", emitList)
  ...
  watchFiles(watchPath, watchOptions) {
    info("webpack-dev-server watchFiles watchPath:" + watchPath + " watchOptions:", watchOptions)
    ...
    if (this.options.liveReload) {
      watcher.on('change', (item) => {
        info("webpack-dev-server watchFiles change:", item)
        this.sendMessage({
          messages: [
            {
  ...
  async start() {
    ...
    await new Promise((resolve) => {
      info("webpack-dev-server server.listen start...")
      this.server.listen(this.options.port, this.options.host, () => {
        info("webpack-dev-server server.listen end...")
        resolve();
      });
    });
