...
function createInvoker(initialValue: EventValue, instance: ComponentInternalInstance | null) {
  const invoker = (e: Event) => {
    callWithAsyncErrorHandling(invoker.value as Invoker, instance, 5, [e]);
  };
...
export function patchEvent(
  ...
  info("patchEvent: ", rawEl, rawName, prevValue, nextValue);
  ...
  if (nextValue && existingInvoker) {
    ...
    if (nextValue) {
      ...
      el.addEventListener(name as string, invoker as Invoker, options as EventOption);
    } else {
      // remove the event, first remove the event from el
      el.removeEventListener(name as string, existingInvoker as Invoker, options as EventOption);
