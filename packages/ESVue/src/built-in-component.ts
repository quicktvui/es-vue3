...
import { HIPPY_DEBUG_ADDRESS, NATIVE_COMPONENT_MAP } from "./config";
import { ElementComponent, registerElement } from "./runtime/component";
import type { EventsUnionType } from "./runtime/event/hippy-event";
import { Native } from "./runtime/native";
import type { HippyNode } from "./runtime/node/hippy-node";
import { arrayCount, convertImageLocalPath, mapHippyEvent, warn } from "./util";
import { HippyEventChild } from "./runtime/event/hippy-event";
    ...
    processEventData(evtData: EventsUnionType, nativeEventParams: NeedToTyped) {
      ...
      switch (nativeEventName) {
        ...
        case "onLoadEnd":
          ...
          //TODO LIULIPENG 20250703 不可以删，用于处理图片的自动宽高
          event.image = nativeEventParams.image;
          break;
        default:
      }
      return event;
    ...
    processEventData(
      ...
      switch (nativeEventName) {
        ...
        case "onDelete":
          ...
          break;
        case "onScrollOffset":
          event.offsetX = nativeEventParams.x;
          event.offsetY = nativeEventParams.y;
          break;
        case "onScrollStateChanged":
          event.offsetX = nativeEventParams.contentOffset.x;
          event.offsetY = nativeEventParams.contentOffset.y;
          if (nativeEventParams.state) {
            if (nativeEventParams.state.oldState) {
              event.oldState = nativeEventParams.state.oldState;
            } else {
              event.oldState = 0;
            }

            if (nativeEventParams.state.newState) {
        ...
        case "onFocusSearchFailed":
          const childEvent: HippyEventChild = {
            index: nativeEventParams.child.index,
            id: nativeEventParams.child.id,
            name: nativeEventParams.child.name,
            position: nativeEventParams.child.position,
          };
          event.child = childEvent;
          event.focused = childEvent;

          if (nativeEventParams.direction) {
