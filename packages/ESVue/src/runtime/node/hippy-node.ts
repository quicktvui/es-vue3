 ...
 */
import type { CallbackType, NativeNode, NativeNodeProps } from "../../types";
import { DEFAULT_ROOT_ID, getUniqueId } from "../../util";
...
export class HippyNode extends HippyEventTarget {
  ...
  public get index(): number {
    ...
    if (this.parentNode) {
      ...
      const needInsertToNativeChildNodes: HippyNode[] = this.parentNode.childNodes.filter(
        (node) => node.isNeedInsertToNative,
      );
  ...
  public insertBefore(rawChild: HippyNode, rawAnchor: HippyNode | null): void {
    info("[HippyNode] insertBefore", rawChild, rawAnchor);
  ...
  public moveChild(rawChild: HippyNode, rawAnchor: HippyNode | null): void {
    info("[HippyNode] moveChild", rawChild, rawAnchor);
    const child = rawChild;
    const anchor = rawAnchor;
  ...
  public removeChild(rawChild: HippyNode): void {
    info("[HippyNode] removeChild", rawChild);
    const child = rawChild;
  ...
  public insertChildNativeNode(child: HippyNode, refInfo: HippyTypes.ReferenceInfo = {}): void {
    info("[HippyNode] insertChildNativeNode", child);
  ...
  public removeChildNativeNode(child: HippyNode): void {
    info("[HippyNode] removeChildNativeNode", child);
    if (!child || !child.isNeedInsertToNative) return;
  ...
  public updateNativeNode(isIncludeChildren = false): void {
    info("[HippyNode] updateNativeNode", this, " isIncludeChildren:", isIncludeChildren);
    ...
    // get native nodes
    const updateNodes: convertToNativeNodesReturnedVal = this.convertToNativeNodes(
      isIncludeChildren,
      {},
    );
  ...
  public convertToNativeNodes(
    ...
    const nativeNode = {
      ...
      index: this.index, //hippy 2.x 必须需要
      ...(IS_PROD
        ? {}
        : {
    ...
    if (IS_HIPPY_3) {
      ...
      return [[nativeNode], [eventNode], [printedNode]];
    }
  }
}
