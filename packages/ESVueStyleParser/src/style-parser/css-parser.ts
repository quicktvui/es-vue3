...
function parseBackground(property: string, value: NeedToTyped): [string, NeedToTyped] {
  ...
  for (const layer of layers) {
    if (layer.indexOf("linear-gradient") === 0) {
      ...
      const valueString = layer.substring(layer.indexOf("(") + 1, layer.lastIndexOf(")"));
 ...
 * @public
 */
function parseBackgroundImage(property: string, value: NeedToTyped): [string, NeedToTyped] {
  let processedValue = value;
  let processedProperty = property;
  const layers = splitLayers(processedValue);
  for (const layer of layers) {
 ...
 * @public
 */
function parseCSS(css: string, options: CssParserOption = { source: 0 }): CssNode {
  // position
  let lineno = 1;
  let column = 1;
...
function parseBorderRadius(property: string, value: any): Record<string, any> {
  const parts = typeof value === "number" ? [value.toString()] : value.trim().split(/\s+/);
...
function parseFocusBorderRadius(property: string, value: any): Record<string, any> {
  const parts = typeof value === "number" ? [value.toString()] : value.trim().split(/\s+/);
...
//-------------------------------------主题变量的解析-------------------------------------
function normalizeThemeVariables(theme: Record<string, string>): Record<string, any> {
  ...
  switch (property) {
    ...
    case "shadowOffset": {
      const pos = value
        ...
        .map((v) => convertPxUnitToPt(v));
      const [x, y = x] = pos;
      return property === "textShadowOffset" ? { width: x, height: y } : { x, y };
    }
