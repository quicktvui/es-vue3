...
import { playerViewManagerKey } from "../injectionSymbols";
...
export function createESPlayerViewManager(): ESPlayerViewManager {
  let _enabled = true;
  let _listenerList: Array<ESIPlayerView> = [];
  ...
  function addListener<T extends ESIPlayerView>(listener: T): void {
    // 去重
  ...
  function removeListener<T extends ESIPlayerView>(listener: T): void {
  ...
  return {
    install: function (app: App) {
      const instance = this;
      app.provide(playerViewManagerKey, instance);
    },
    init,
    addListener,
    removeListener,
