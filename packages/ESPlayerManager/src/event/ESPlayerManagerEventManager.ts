 ...
 * 播放器监听管理者
 */
export interface ESPlayerManagerEventManager {
  addListener<T extends ESPlayerManagerEventListener>(listener: T): void;

  removeListener<T extends ESPlayerManagerEventListener>(listener: T): void;
...
export function createESPlayerManagerEventManager(): ESPlayerManagerEventManager {
  let _enabled = true;
  let _listenerList: Array<ESPlayerManagerEventListener> = [];
  let _player: ESIPlayerManager;

  function addListener<T extends ESPlayerManagerEventListener>(listener: T): void {
    // 去重
  ...
  function removeListener<T extends ESPlayerManagerEventListener>(listener: T): void {
  ...
  function onPlayerSizeChanged(): void {
    playerEventMethodEval("onPlayerSizeChanged");
  }
